...
for i in range(0, n_plots):
    plt.figure(i, figsize=(9, 3), dpi=150)
    ...
    plt.plot(dr_error, label=f'DR error', color=dr_color)
    plt.plot(online_error, label=f'Online error', color=data_colors[i])

    if plot_crossings:
        crossing_labeled = False
        for crossing in crossings[i]:
            plt.axvline(crossing, color='magenta', linestyle='--')
    ...
    if plot_buoy_detections:
        for detection_ignore_ind, detection_ind in enumerate(detections_inds):
            if detection_ignore_ind not in detections_ignore:
                plt.axvline(detection_ind, color='orange', linestyle='--')
