...
class Simulation:
    ...
    def plot_detection_zone(self,pose, detect_range, detect_half_angle, color):
        ...
        # due to symetry the other detection zone can by found from the relative positions above
    ...
    def write_text(self, text, font_size, position, screen, center=False):
...
class SAM_sim:
    ...
    def generate_noisy_path(self):
        ...
        # Use the new noisy odometrey (with noise) to compute the ground truth
    ...
    def generate_simulated_detections(self):
        ...
        # [array index at which detection occured, distance, relative bearing, landmark id]
        # The landmark shouldnt be used except for checking!!!
        ...
        for ind_lm, lm_state in enumerate(self.sim.farm_buoys):
            ...
            if abs(rel_bearing) <= (np.pi/2 + self.detect_half_angle) and abs(rel_bearing) >= (np.pi/2 - self.detect_half_angle):
