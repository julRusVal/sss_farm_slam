...
"""
Plot factor graph of method 3, no rope detections

Rope detections are still added to the graph but no prior constraint is applied. This means that the rope detection
itself will not effect the estimate but still allow for plotting of the rope detections.
"""
...
# Rope prior(s)
# Rope priors unused in method 3
# rope_prior_factor_0 is used only to make the sizes of the plot the same between the three methods
# r_0
...
node_colors = {
    ...
    'r_0': rope_color,
    'r_1': rope_color,
    rope_prior_factor_0: blank_color,
...
# Define edge colors
blank_list = ['BLANK', rope_prior_factor_0]
node_edge_colors = {node: 'black' if node not in blank_list else 'white' for node in G.nodes()}
